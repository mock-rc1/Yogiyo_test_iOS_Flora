...
class TangViewController: UIViewController, IndicatorInfoProvider {
    ...
    lazy var dataManager: CategoryStoreDataManager6 = CategoryStoreDataManager6()

   var categoryStore : [CategoryStoreResult] = []
    
    @IBOutlet weak var SoupTableView: UITableView!
    ...
    var sections = [1]
        ...
        override func viewDidLoad() {
            ...
            self.SoupTableView.register(MyStoreTableViewCell.nib(), forCellReuseIdentifier: MyStoreTableViewCell.identifier)
            
            SoupTableView.separatorColor = UIColor.clear
            
                    //AllTableView.tableFooterView = UIView(frame: .zero)
            SoupTableView.sectionFooterHeight = 0
            
            dataManager.getCategoryStore6(delegate: self)
        }
        ...
        func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
            return IndicatorInfo(title: "찜/탕")
        }
    
    func setDelegate() {
        SoupTableView.delegate = self
        SoupTableView.dataSource = self
    }
    func setStyle() {
        // 테이블 뷰 경계션 없애기
        SoupTableView.separatorInset = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: .greatestFiniteMagnitude)
    }

}

extension TangViewController: UITableViewDelegate, UITableViewDataSource {
