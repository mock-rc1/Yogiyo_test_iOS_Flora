...
class BunsikViewController: UIViewController, IndicatorInfoProvider {
    ...
    lazy var dataManager: CategoryStoreDataManager8 = CategoryStoreDataManager8()

   var categoryStore : [CategoryStoreResult] = []
    
    @IBOutlet weak var bunsikTableView: UITableView!
    ...
    var sections = [1]
        ...
        override func viewDidLoad() {
            ...
            self.bunsikTableView.register(MyStoreTableViewCell.nib(), forCellReuseIdentifier: MyStoreTableViewCell.identifier)
            
            bunsikTableView.separatorColor = UIColor.clear
            
                    //AllTableView.tableFooterView = UIView(frame: .zero)
            bunsikTableView.sectionFooterHeight = 0
            
            dataManager.getCategoryStore8(delegate: self)
        }
    ...
    func setDelegate() {
        bunsikTableView.delegate = self
        bunsikTableView.dataSource = self
    }
    func setStyle() {
        // 테이블 뷰 경계션 없애기
        bunsikTableView.separatorInset = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: .greatestFiniteMagnitude)
    }


    }

extension BunsikViewController: UITableViewDelegate, UITableViewDataSource {
