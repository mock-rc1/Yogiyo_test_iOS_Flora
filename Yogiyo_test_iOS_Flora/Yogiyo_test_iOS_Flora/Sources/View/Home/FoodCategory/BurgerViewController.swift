...
class BurgerViewController: UIViewController, IndicatorInfoProvider {

    lazy var dataManager: CategoryStoreDataManager4 = CategoryStoreDataManager4()

    var categoryStore : [CategoryStoreResult] = []
    
    @IBOutlet weak var BurgerTableView: UITableView!
    ...
    override func viewDidLoad() {
        ...
        self.BurgerTableView.register(MyStoreTableViewCell.nib(), forCellReuseIdentifier: MyStoreTableViewCell.identifier)
        
        BurgerTableView.separatorColor = UIColor.clear
        
                //AllTableView.tableFooterView = UIView(frame: .zero)
        BurgerTableView.sectionFooterHeight = 0
        
        dataManager.getCategoryStore4(delegate: self)
    ...
    func setStyle() {
        // 테이블 뷰 경계션 없애기
        BurgerTableView.separatorInset = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: .greatestFiniteMagnitude)
    }
}
