...
class KoreaFoodViewController: UIViewController, IndicatorInfoProvider {

    lazy var dataManager: CategoryStoreDataManager3 = CategoryStoreDataManager3()

    var categoryStore : [CategoryStoreResult] = []
    
    @IBOutlet weak var KoreaFoodTableView: UITableView!
    ...
    var sections = [1]
        ...
        override func viewDidLoad() {
            ...
            self.KoreaFoodTableView.register(MyStoreTableViewCell.nib(), forCellReuseIdentifier: MyStoreTableViewCell.identifier)
            
            KoreaFoodTableView.separatorColor = UIColor.clear
            
                    //AllTableView.tableFooterView = UIView(frame: .zero)
            KoreaFoodTableView.sectionFooterHeight = 0
            
            dataManager.getCategoryStore3(delegate: self)
        }
        ...
        func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
            return IndicatorInfo(title: "한식")
        }
    
    func setDelegate() {
        KoreaFoodTableView.delegate = self
        KoreaFoodTableView.dataSource = self
    }
    func setStyle() {
        // 테이블 뷰 경계션 없애기
        KoreaFoodTableView.separatorInset = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: .greatestFiniteMagnitude)
    }

}

extension KoreaFoodViewController: UITableViewDelegate, UITableViewDataSource {
