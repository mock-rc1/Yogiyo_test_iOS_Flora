...
class OnePersonViewController: UIViewController, IndicatorInfoProvider {

    lazy var dataManager: CategoryStoreDataManager9 = CategoryStoreDataManager9()

   var categoryStore : [CategoryStoreResult] = []
    
    @IBOutlet weak var OnePersonTableView: UITableView!
    ...
    var sections = [1]
        ...
        override func viewDidLoad() {
            ...
            self.OnePersonTableView.register(MyStoreTableViewCell.nib(), forCellReuseIdentifier: MyStoreTableViewCell.identifier)
            
            OnePersonTableView.separatorColor = UIColor.clear
            
                    //AllTableView.tableFooterView = UIView(frame: .zero)
            OnePersonTableView.sectionFooterHeight = 0
            
            dataManager.getCategoryStore9(delegate: self)
        }
        ...
        func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
            return IndicatorInfo(title: "1인분주문")
        }
    
    func setDelegate() {
        OnePersonTableView.delegate = self
        OnePersonTableView.dataSource = self
    }
    func setStyle() {
        // 테이블 뷰 경계션 없애기
        OnePersonTableView.separatorInset = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: .greatestFiniteMagnitude)
    }

    }
extension OnePersonViewController: UITableViewDelegate, UITableViewDataSource {
