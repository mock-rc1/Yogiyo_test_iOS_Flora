...
class BossamViewController: UIViewController, IndicatorInfoProvider {
    ...
    lazy var dataManager: CategoryStoreDataManager12 = CategoryStoreDataManager12()
   ...
   var categoryStore : [CategoryStoreResult] = []
    ...
    var sections = [1]
        ...
        override func viewDidLoad() {
            ...
            self.pigTableView.register(MyStoreTableViewCell.nib(), forCellReuseIdentifier: MyStoreTableViewCell.identifier)
            
            pigTableView.separatorColor = UIColor.clear
            
                    //AllTableView.tableFooterView = UIView(frame: .zero)
            pigTableView.sectionFooterHeight = 0
            
            dataManager.getCategoryStore12(delegate: self)
        }
    ...
    func setDelegate() {
        pigTableView.delegate = self
        pigTableView.dataSource = self
    }
    func setStyle() {
        // 테이블 뷰 경계션 없애기
        pigTableView.separatorInset = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: .greatestFiniteMagnitude)
    }

    }

extension BossamViewController: UITableViewDelegate, UITableViewDataSource {
