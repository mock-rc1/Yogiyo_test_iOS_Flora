...
class JapaneseFoodViewController: UIViewController, IndicatorInfoProvider {
    ...
    lazy var dataManager: CategoryStoreDataManager10 = CategoryStoreDataManager10()

   var categoryStore : [CategoryStoreResult] = []
    
    @IBOutlet weak var japanTableView: UITableView!
    ...
    var sections = [1]
        ...
        override func viewDidLoad() {
            ...
            self.japanTableView.register(MyStoreTableViewCell.nib(), forCellReuseIdentifier: MyStoreTableViewCell.identifier)
            
            japanTableView.separatorColor = UIColor.clear
            
                    //AllTableView.tableFooterView = UIView(frame: .zero)
            japanTableView.sectionFooterHeight = 0
            
            dataManager.getCategoryStore10(delegate: self)
        }
    ...
    func setStyle() {
        // 테이블 뷰 경계션 없애기
        japanTableView.separatorInset = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: .greatestFiniteMagnitude)
    }

    }

extension JapaneseFoodViewController: UITableViewDelegate, UITableViewDataSource {
