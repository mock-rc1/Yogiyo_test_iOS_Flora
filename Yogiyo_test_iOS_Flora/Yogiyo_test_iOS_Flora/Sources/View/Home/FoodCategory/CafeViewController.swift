...
class CafeViewController: UIViewController, IndicatorInfoProvider {
    ...
    lazy var dataManager: CategoryStoreDataManager7 = CategoryStoreDataManager7()
   ...
   var categoryStore : [CategoryStoreResult] = []
    ...
    var sections = [1]
        ...
        override func viewDidLoad() {
            ...
            self.cafeTableView.register(MyStoreTableViewCell.nib(), forCellReuseIdentifier: MyStoreTableViewCell.identifier)
            
            cafeTableView.separatorColor = UIColor.clear
            
                    //AllTableView.tableFooterView = UIView(frame: .zero)
            cafeTableView.sectionFooterHeight = 0
            
            dataManager.getCategoryStore7(delegate: self)
        }
        ...
        func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
            return IndicatorInfo(title: "카페/디저트")
        }
    ...
    func setStyle() {
        // 테이블 뷰 경계션 없애기
        cafeTableView.separatorInset = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: .greatestFiniteMagnitude)
    }
